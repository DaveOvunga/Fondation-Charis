                            ...
                            <div>
                                <nav aria-label="breadcrumb ">
                                    <ol class="breadcrumb p-0 bg-dark">
                                        <li class="breadcrumb-item"><a href="<?=APP_URL?>/formation">Accueil</a></li>
                                        <li class="breadcrumb-item"><a href="<?php echo APP_URL.'/formation/course/'. htmlspecialchars(base64_encode($data['course_id']))?>"><?php echo htmlspecialchars($data['course_name']); ?></a></li>
    ...
    <script>
        $(document).ready(function() {
            $('#nextChapBtn').on('click', function(event) {
                ...
                $.ajax({
                    url: '<?php echo APP_URL?>/formation/chaptercomplete', 
                    type: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        if (response.success && response.redirect) {
                            ...
                            setTimeout(function() {
                                window.location.href = "<?php echo APP_URL?>/formation/course/<?php echo Helper::encodeIds($data['course_id'],$data['next_chapter_id']) ?>/video" // Redirect to the login page
